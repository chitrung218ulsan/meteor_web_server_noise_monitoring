...
Template.checkDataSound.helpers({
	checkNormal:function(){
		...
		var wanrningSoundThreshold = parseFloat(buildingSelected.warningSoundThreshold);
        	var valueCheck = parseFloat(home.sound);
		
		if(valueCheck < wanrningSoundThreshold )
		{
	...
	checkWarning:function(){
		...
		var wanrningSoundThreshold = parseFloat(buildingSelected.warningSoundThreshold);
		var dangerSoundThreshold = parseFloat(buildingSelected.dangerSoundThreshold);

		if(wanrningSoundThreshold  <= valueCheck &&  valueCheck < dangerSoundThreshold )
		{
	...
	checkDanger:function(){
		...
		var dangerSoundThreshold = parseFloat(buildingSelected.dangerSoundThreshold);
        	...
        	var valueCheck = parseFloat(home.sound);
		if(dangerSoundThreshold  < valueCheck )
		{
			return true;
		}
		else 
